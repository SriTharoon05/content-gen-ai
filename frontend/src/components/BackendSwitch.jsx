  ...
  return <details className="card" style={{ marginBottom: 16 }}>
    ...
    <div className="grid cols-2" style={{ marginTop: 16 }}>
      <label>Backend<select value={backend} onChange={e => { setBackend(e.target.value); setToken(backendToken(e.target.value)) }}>
        <option value="render">Render</option><option value="cloudflare">Cloudflare — generation preview</option>
      </select></label>
      <label>Admin token for {backend}<input type="password" autoComplete="off" value={token} onChange={e => setToken(e.target.value)} /></label>
    </div>
    <p className="muted tiny">Saved on this browser. Existing jobs and scheduled runs are not moved. Cloudflare currently supports English single-narrator generation and preview, not full production feature parity.</p>
